...
8. Connect to your SensorTag device using its MAC address by entering **connect \<MAC address>**. Note that the sample output below is abbreviated:
    ...
    > Note: You can list the GATT (Generic Attribute Profile) characteristics of the device again using the **list-attributes** command. For the full SensorTag 2.0 GATT table please refer to [this page](http://www.ti.com/ww/en/wireless_connectivity/sensortag/tearDown.html) - scroll it all the way down. To gain a better understanding of all the sensors in your TI Tag feel free to install the TI SensorTag app on your phone and  explore.
