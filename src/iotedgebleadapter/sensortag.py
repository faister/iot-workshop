...
def main():
	...
	parser.add_argument('-n', action='store', dest='count',
						default=0, type=int, help="Number of times to loop data")
	parser.add_argument('-t', action='store', type=float,
						default=1.0, help='time between polling')
	parser.add_argument('--redisip', action='store',
						default='127.0.0.1', help='Redis IP address')
	parser.add_argument('--redisport', action='store',
						default='6379', help='Redis port')
	parser.add_argument('--redispassword', action='store',
						default=None, help='Redis password')

	parser.add_argument('--redischannel', action='store',
						default='data', help='Redis pub/sub channel')
	...
	parser.add_argument('-M', '--magnetometer',
						action='store_true', default=False)
	...
	r = redis.StrictRedis(host=arg.redisip, port=arg.redisport,
						  password=arg.redispassword) or sys.exit("Error connecting to Redis")
	...
	while True:
		if arg.temperature or arg.all:
			val = tag.IRtemperature.read()
			print('Temp: ', val)
			data = {'tag': arg.host, 'temp_ambient': val[0], 'temp_ir': val[1], 
					'timestamp': datetime.datetime.utcnow().replace(microsecond=0).isoformat()}
			r.publish(arg.redischannel, json.dumps(data))
		if arg.humidity or arg.all:
			val = tag.humidity.read()
			print("Humidity: ", val)
			data = {'tag': arg.host, 'humidity_temp_ambient': val[0], 'relative_humidity': val[1], 
					'timestamp': datetime.datetime.utcnow().replace(microsecond=0).isoformat()}
			r.publish(arg.redischannel, json.dumps(data))
		if arg.barometer or arg.all:
			val = tag.barometer.read()
			print("Barometer: ", val)
			data = {'tag': arg.host, 'barometer_temp_ambient': val[0], 'pressure_millibars': val[1], 
					'timestamp': datetime.datetime.utcnow().replace(microsecond=0).isoformat()}
			r.publish(arg.redischannel, json.dumps(data))
		if arg.accelerometer or arg.all:
			val = tag.accelerometer.read()
			print("Accelerometer: ", val)
		...
		if arg.magnetometer or arg.all:
			...
			data = {'tag': arg.host, 'magnetometer_x': val[0], 'magnetometer_y': val[1], 'magnetometer_z': val[2],
					'timestamp': datetime.datetime.utcnow().replace(microsecond=0).isoformat()}
			r.publish(arg.redischannel, json.dumps(data))
		if arg.gyroscope or arg.all:
			val = tag.gyroscope.read()
			print("Gyroscope: ", val)
			data = {'tag': arg.host, 'gyroscope_x': val[0], 'gyroscope_y': val[1], 'gyroscope_z': val[2],
					'timestamp': datetime.datetime.utcnow().replace(microsecond=0).isoformat()}
			r.publish(arg.redischannel, json.dumps(data))
		if (arg.light or arg.all) and tag.lightmeter is not None:
